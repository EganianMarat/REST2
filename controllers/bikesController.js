...
bikeController.createCar = app.post('/', (req, res) => {
	let dataNew = [];
...
bikeController.getCars = app.get('/', (req, res) => {
...
bikeController.updateCar = app.put('/:id', (req, res) => {
...
bikeController.deleteCar = app.delete('/:id', (req, res) => {
	if(fs.existsSync(dataBase) && req.params.id) {
		let dataNew = [];
		dataNew = JSON.parse(fs.readFileSync(dataBase));
		let key = -1;
