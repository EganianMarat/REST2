...
carController.createCar = app.post('/', (req, res) => {
	let dataNew = [];
...
carController.getCars = app.get('/', (req, res) => {
...
carController.updateCar = app.put('/:id', (req, res) => {
...
carController.deleteCar = app.delete('/:id', (req, res) => {
	if(fs.existsSync(dataBase) && req.params.id) {
		let dataNew = [];
		dataNew = JSON.parse(fs.readFileSync(dataBase));
		let key = -1;
